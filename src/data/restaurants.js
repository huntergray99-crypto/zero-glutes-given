// Zero Glutes Given — seed dataset
// Community-compiled from the Seattle celiac/gluten-free community, Find Me Gluten
// Free reports, dedicated-celiac blogs (Wheatless Wanderlust, The Celiac MD), and
// restaurants' own published allergen info. Verified against the web in Aug 2026:
// closed spots removed, addresses/phones/safety levels corrected. Still a starting
// point, not a guarantee — ALWAYS confirm your needs with the restaurant.
//
// safetyLevel:
//   "dedicated"        - 100% gluten-free facility, no gluten on premises
//   "celiac-friendly"  - gluten in the kitchen, but documented separate-prep
//                        protocols and staff trained on cross-contamination
//   "gf-menu"          - marked GF menu/options, shared kitchen, ask about protocols
//   "honorable"        - NOT a gluten-free kitchen. Listed because it's healthy,
//                        affordable, and has lots of naturally gluten-free options.
//                        Cross-contamination is likely — a low-risk quick bite,
//                        not a celiac-safe meal. Hidden until you opt in.
//
// featured  - standout spot the GF community rallies around (gold on the map)
// spotlight - local hero we especially want people to know about

export const CUISINES = [
  'American',
  'Italian',
  'Mediterranean',
  'Pizza',
  'Mexican',
  'Asian',
  'BBQ',
  'Seafood',
  'Bowls & Salads',
  'Bakery & Sweets',
  'Brunch',
  'Vegan & Veggie',
  'Bars & Breweries',
  'Hawaiian',
];

export const restaurants = [
  {
    id: 'the-angry-beaver',
    featured: true,
    spotlight: true,
    name: 'The Angry Beaver',
    neighborhood: 'Greenwood',
    address: '8412 Greenwood Ave N, Seattle, WA 98103',
    lat: 47.6905,
    lng: -122.3549,
    cuisine: ['Bars & Breweries', 'American'],
    safetyLevel: 'dedicated',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://theangrybeaverseattle.com',
    phone: '(206) 739-5959',
    summary:
      "A Canadian hockey bar that went fully gluten-free — the kitchen is now 100% GF with dedicated equipment and a dedicated fryer, and it's one of the safest bars in Seattle for celiacs. Watch a game and actually relax.",
    whatToOrder:
      'Poutine, the hot link corn dog in tempura batter, wings, the Angry Beaver burger.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Kitchen till midnight Fri & Sat, 10pm the rest of the week',
    getThere: {
      transit: 'Metro Route 5 stops right outside on Greenwood Ave N; RapidRide E on Aurora is a 5-min walk',
      bike: 'Greenwood Ave N bike lane out front; the Interurban Trail is a few blocks west',
      parking: 'Free street parking on Greenwood Ave and the side streets — usually easy except a Kraken game night',
    },
  },
  {
    id: 'ghostfish-brewing',
    featured: true,
    name: 'Ghostfish Brewing Company',
    neighborhood: 'SODO',
    address: '2942 1st Ave S, Seattle, WA 98134',
    lat: 47.5793,
    lng: -122.3339,
    cuisine: ['Bars & Breweries', 'American'],
    safetyLevel: 'dedicated',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.ghostfishbrewing.com',
    phone: '(206) 397-3898',
    summary:
      "Washington's first gluten-free brewery. The taproom kitchen and every beer are 100% gluten-free — nothing with gluten is in the building.",
    whatToOrder:
      'Beer-battered fish & chips, fried chicken sandwich, pretzel bites. The fryer is dedicated GF.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Taproom till 10pm Fri & Sat, 9pm weeknights; kitchen stops 15 min before close',
    getThere: {
      transit: 'SODO Link light rail station is a ~10-min walk; buses run on 1st Ave S',
      bike: 'The SODO Trail runs one block east on Occidental Ave S',
      parking: 'Free street parking, plentiful in SODO on evenings and weekends',
    },
  },
  {
    id: 'a-stir',
    featured: true,
    name: 'a/stir',
    neighborhood: 'Capitol Hill',
    address: '818 E Pike St, Seattle, WA 98122',
    lat: 47.6142,
    lng: -122.319,
    cuisine: ['Mediterranean', 'Italian'],
    safetyLevel: 'dedicated',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 3,
    website: 'https://astirseattle.com',
    phone: '(206) 397-3564',
    summary:
      'The former Capitol Cider, reborn in 2023 as a Northern Mediterranean restaurant and bar that kept the all-gluten-free kitchen. Contemporary Spanish/French/Italian/Greek/Turkish small plates, all GF, with knowledgeable staff. Dinner only, Tue–Sun.',
    whatToOrder: 'Whatever the seasonal menu is doing with seafood; the fried items from the dedicated GF fryer.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Speakeasy bar till 1am Wed and 2am Fri & Sat — the rare fully gluten-free late-night spot in Seattle',
    getThere: {
      transit: 'Capitol Hill Link station is a 5-min walk; many buses on Pike, Pine, and Broadway',
      bike: 'Pike/Pine bike lanes out front; the Broadway protected bikeway is one block east',
      parking: 'Street only and paid until 10pm ($6–8/hr in Pike/Pine) — rough on weekend nights. Use a pay lot or come by transit',
    },
  },
  {
    id: 'the-chicken-supply',
    featured: true,
    name: 'The Chicken Supply',
    neighborhood: 'Phinney Ridge',
    address: '7410 Greenwood Ave N, Seattle, WA 98103',
    lat: 47.6828,
    lng: -122.355,
    cuisine: ['American', 'Asian'],
    safetyLevel: 'dedicated',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.thechickensupply.com',
    phone: '(206) 453-4172',
    summary:
      'Filipino fried chicken dredged in tapioca, rice, and potato starch — the entire menu is gluten-free. Named to the New York Times list of Seattle\'s best restaurants. Open Wed–Sun until they sell out; pre-order.',
    whatToOrder: 'Fried chicken (whole or by the piece), garlic rice, mac salad.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 5 on Greenwood Ave N; RapidRide E on Aurora is a ~7-min walk',
      bike: 'Greenwood Ave N bike route runs past the door',
      parking: 'Free street parking on Greenwood and the side streets',
    },
  },
  {
    id: 'nuflours',
    featured: true,
    name: 'Nuflours Bakery',
    neighborhood: 'Capitol Hill',
    address: '518 15th Ave E, Seattle, WA 98112',
    lat: 47.6234,
    lng: -122.3122,
    cuisine: ['Bakery & Sweets'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://nuflours.com',
    phone: '(206) 395-4623',
    summary:
      "Seattle's favorite dedicated gluten-free bakery since 2011 — breads, bagels, croissants, quiche, and celebration cakes. Open Mon–Sat.",
    whatToOrder: 'Sandwich bread loaves, morning buns, and the seasonal galettes.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 10 stops on 15th Ave E right outside',
      bike: '15th Ave E has sharrows; a short ride from the Capitol Hill core',
      parking: 'Free 2-hr street parking on 15th Ave E and the side streets — usually findable',
    },
  },
  {
    id: 'askatu-bakery',
    featured: true,
    name: 'Askatu Bakery & Cafe',
    neighborhood: 'Belltown',
    address: '2209 4th Ave, Seattle, WA 98121',
    lat: 47.6135,
    lng: -122.3428,
    cuisine: ['Bakery & Sweets'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://liberatedfoods.com',
    phone: '(206) 485-7503',
    summary:
      'Fully allergen-free bakery and cafe — 100% gluten-free, and also free of wheat, egg, and nuts, with many vegan options. Open Tue–Sat.',
    whatToOrder: 'Morning pastries, focaccia, and a cake slice from the case.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'A block from the 3rd Ave bus spine; Westlake Link station is a ~10-min walk',
      bike: 'The 2nd Ave protected bike lane is one block over',
      parking: 'Paid street parking and Belltown garages — free spots are rare here',
    },
  },
  {
    id: 'frankie-and-jos',
    featured: true,
    name: "Frankie & Jo's (Capitol Hill)",
    neighborhood: 'Capitol Hill',
    address: '1010 E Union St, Seattle, WA 98122',
    lat: 47.6131,
    lng: -122.3195,
    cuisine: ['Bakery & Sweets', 'Vegan & Veggie'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://frankieandjos.com',
    phone: '(206) 257-4794',
    summary:
      'Plant-based ice cream shop that is 100% vegan, gluten-free, and soy-free, with gluten-free waffle cones. Note: flavors contain gluten-free oats.',
    whatToOrder: 'Brown Sugar Vanilla or Salty Caramel Ash in a GF cone.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Scooping till 11pm Fri & Sat, 10pm otherwise — a safe late dessert run',
    getThere: {
      transit: 'Metro Route 2 on E Union St; Capitol Hill Link station is a ~8-min walk',
      bike: 'E Union St bike lane; steps from the Pike/Pine routes',
      parking: 'Free street parking on E Union and the side streets, moderate turnover',
    },
  },
  {
    id: 'sankaku-onigiri',
    name: 'sankaku onigiri cafe & bar',
    neighborhood: 'Capitol Hill',
    address: '1527 Melrose Ave, Seattle, WA 98122',
    lat: 47.6146,
    lng: -122.327,
    cuisine: ['Asian'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 1,
    website: 'https://www.sankakuseattle.com',
    phone: '',
    summary:
      'A fully gluten-free Japanese onigiri counter inside Melrose Market. Homestyle rice balls with fillings like ume-konbu and tuna mayo, plus vegan options.',
    whatToOrder: 'A few onigiri, miso soup, and a matcha.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Inside Melrose Market on Capitol Hill — a 6-min walk from the Link station',
      bike: 'Off the Pike/Pine bike lanes; racks along Melrose Ave',
      parking: 'Street only, paid until 10pm — a weeknight lunch is easy, evenings less so',
    },
  },
  {
    id: 'yeobo-cafe',
    name: 'Yeobo Cafe and Bar',
    neighborhood: 'Madison Valley',
    address: '2332 E Madison St, Seattle, WA 98112',
    lat: 47.6234,
    lng: -122.2965,
    cuisine: ['Asian', 'American'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.yeobosea.com',
    phone: '',
    summary:
      'A 100% gluten-free Korean-inspired cafe and bar (opened 2025) — breakfast sandwiches, sandwiches on house GF bread, coffee, wine, and beer. Closed Wednesdays.',
    whatToOrder: 'A gluten-free breakfast sandwich and a coffee, or come back for wine and small plates.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 8 and the 11 run along E Madison St past the door',
      bike: 'E Madison St has bike lanes connecting to the Central District and Capitol Hill',
      parking: 'Free street parking on E Madison and the residential side streets — usually easy',
    },
  },
  {
    id: 'razzis-pizzeria',
    featured: true,
    name: "Razzi's Pizzeria",
    neighborhood: 'Greenwood',
    address: '8523 Greenwood Ave N, Seattle, WA 98103',
    lat: 47.6913,
    lng: -122.3549,
    cuisine: ['Pizza', 'Italian', 'Vegan & Veggie'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.razzispizzeria.com',
    phone: '(206) 782-9005',
    summary:
      'Fully vegetarian pizzeria with a gluten-free menu as long as the regular one: separate GF prep area, dedicated GF fryer, and staff who take celiac seriously.',
    whatToOrder: 'GF pizza, GF calzone, and the fried ravioli from the dedicated fryer.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 5 on Greenwood Ave N stops just outside',
      bike: 'On the Greenwood Ave N bike route',
      parking: 'Free street parking on Greenwood and side streets',
    },
  },
  {
    id: 'esters-enoteca',
    featured: true,
    name: 'Esters Enoteca',
    neighborhood: 'Fremont',
    address: '3416 Fremont Ave N, Seattle, WA 98103',
    lat: 47.6497,
    lng: -122.3502,
    cuisine: ['Italian', 'Bars & Breweries'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.estersenoteca.com',
    phone: '(206) 295-1903',
    summary:
      'An Italian wine bar that reverse-engineered its kitchen so gluten-free is the default and gluten dishes are the outliers — dedicated GF fryers, separate utensils. Pasta, panini, salads. Wed–Sat, dinner.',
    whatToOrder: 'GF pasta of the day, a panino, and a glass of something Italian.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Routes 5, 40, and 62 all run through Fremont within a 2–3 min walk',
      bike: 'The Burke-Gilman Trail is three blocks south; Fremont Ave has a bike route',
      parking: 'Paid 2-hr street parking in the Fremont core; some free spots on the residential side streets, tight on weekend nights',
    },
  },
  {
    id: 'palermo-pizza-pasta',
    name: 'Palermo Pizza & Pasta',
    neighborhood: 'Capitol Hill',
    address: '350 15th Ave E, Seattle, WA 98112',
    lat: 47.6229,
    lng: -122.3122,
    cuisine: ['Italian', 'Pizza'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.mypalermopizza.com',
    phone: '(206) 322-3875',
    summary:
      'Neighborhood Italian spot with a gluten-free crust (medium and large) and GF pasta options. Shared kitchen — flag celiac and ask how they handle prep.',
    whatToOrder: 'GF pizza and the GF baked pasta dishes.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 10 stops on 15th Ave E right outside',
      bike: '15th Ave E sharrows; a short ride from the Capitol Hill core',
      parking: 'Free 2-hr street parking on 15th Ave E and the side streets',
    },
  },
  {
    id: 'cafe-flora',
    name: 'Cafe Flora',
    neighborhood: 'Madison Valley',
    address: '2901 E Madison St, Seattle, WA 98112',
    lat: 47.6262,
    lng: -122.2937,
    cuisine: ['Vegan & Veggie', 'Brunch', 'American'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 3,
    website: 'https://florarestaurantgroup.com',
    phone: '(206) 325-9100',
    summary:
      'Fully vegetarian since 1991, with clearly marked GF dishes and kitchen staff experienced with allergen modifications. Great weekend brunch.',
    whatToOrder: 'Weekend brunch — GF pancakes and the tofu scramble.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 8 and the 11 stop on E Madison St at the door',
      bike: 'E Madison St bike lanes; the Washington Park Arboretum loop is close',
      parking: 'Free street parking on E Madison and the residential side streets',
    },
  },
  {
    id: 'musang-beacon-hill',
    name: 'Musang',
    neighborhood: 'Beacon Hill',
    address: '2524 Beacon Ave S, Seattle, WA 98144',
    lat: 47.5793,
    lng: -122.3112,
    cuisine: ['Asian', 'American'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 3,
    website: 'https://www.musangseattle.com',
    phone: '(206) 762-4004',
    summary:
      "Chef Melissa Miranda's Filipinx restaurant. Note celiac on your reservation and the host brings a dedicated celiac menu with cross-contamination removed. Reservations recommended.",
    whatToOrder: 'Whatever is on the celiac menu that night — adobo, rice dishes, ube dessert.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'A short walk from Beacon Hill Link station; Route 36 runs on Beacon Ave S',
      bike: 'The Beacon Ave S bike lane runs right past it',
      parking: 'Free street parking on Beacon Ave and the side streets',
    },
  },
  {
    id: 'bamboo-sushi-uvillage',
    name: 'Bamboo Sushi',
    neighborhood: 'University Village',
    address: '2634 NE Village Ln, Seattle, WA 98105',
    lat: 47.6631,
    lng: -122.2989,
    cuisine: ['Asian', 'Seafood'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: true,
    celiacVerified: false,
    priceLevel: 3,
    website: 'https://bamboosushi.com',
    phone: '(206) 566-5668',
    summary:
      'Sustainable sushi where almost the entire menu is gluten-free — tamari by default, a dedicated tempura fryer with no cross-contamination, and only a few items containing wheat. Tell your server celiac.',
    whatToOrder: 'Nigiri and sashimi, GF tempura, the green tea mochi.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Many Metro routes serve University Village; the U District Link station is a ~15-min walk',
      bike: 'The Burke-Gilman Trail runs along the south edge of U Village',
      parking: 'Free 2-hr parking in the U Village lots and garage',
    },
  },
  {
    id: 'jacks-bbq',
    name: "Jack's BBQ",
    neighborhood: 'SODO',
    address: '3924 Airport Way S, Seattle, WA 98108',
    lat: 47.5726,
    lng: -122.3195,
    cuisine: ['BBQ', 'American'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://jacksbbq.com',
    phone: '(206) 467-4038',
    summary:
      'Central Texas barbecue just north of Georgetown. Brisket, ribs, and beef ribs are naturally gluten-free and the sauces are GF. Not a dedicated GF kitchen; skip anything breaded and confirm the sausage.',
    whatToOrder: 'Brisket plate with beans and slaw.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 124 runs on Airport Way S; a long walk from Link, so bus or drive',
      bike: 'Airport Way S has a bike lane linking SODO to Georgetown',
      parking: 'Free parking lot on site plus easy street parking',
    },
  },
  {
    id: 'taylor-shellfish-melrose',
    name: 'Taylor Shellfish Oyster Bar (Melrose)',
    neighborhood: 'Capitol Hill',
    address: '1521 Melrose Ave, Seattle, WA 98122',
    lat: 47.6146,
    lng: -122.327,
    cuisine: ['Seafood'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 3,
    website: 'https://www.taylorshellfishfarms.com',
    phone: '(206) 501-4321',
    summary:
      "Raw bar from the region's biggest shellfish grower, inside Melrose Market. Oysters, clams, mussels, crab, and ceviche are naturally gluten-free. Bread is served alongside — ask for a separate plate.",
    whatToOrder: 'A dozen oysters, steamed clams (confirm the broth), Dungeness crab.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Inside Melrose Market — a 6-min walk from the Capitol Hill Link station',
      bike: 'Off the Pike/Pine bike lanes; racks on Melrose Ave',
      parking: 'Street only, paid until 10pm; the Pike/Pine pay lots are the fallback',
    },
  },
  {
    id: 'marination-ma-kai',
    name: 'Marination Ma Kai',
    neighborhood: 'West Seattle',
    address: '1660 Harbor Ave SW, Seattle, WA 98126',
    lat: 47.5909,
    lng: -122.3776,
    cuisine: ['Hawaiian', 'Asian'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://marinationmobile.com',
    phone: '(206) 328-8226',
    summary:
      'Hawaiian-Korean spot on the water with GF items clearly marked. Shared fryer — stick to the grill items and rice bowls and ask about sauces. Best view in town.',
    whatToOrder: 'Kalua pork or tofu rice bowls, no mac salad. Skip anything fried.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'The West Seattle Water Taxi lands a short walk away; Route 775 shuttle connects the dock',
      bike: 'Right on the Alki Trail — an easy ride from the water taxi or Alki',
      parking: 'Free lot next door and street parking along Harbor Ave SW',
    },
  },
  {
    id: 'ba-bar-capitol-hill',
    name: 'Ba Bar',
    neighborhood: 'Capitol Hill',
    address: '550 12th Ave, Seattle, WA 98122',
    lat: 47.6086,
    lng: -122.3168,
    cuisine: ['Asian'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.babarseattle.com',
    phone: '(206) 328-2030',
    summary:
      'Modern Vietnamese with GF items marked. Important for celiacs: they cook all noodles and dumplings in shared water and will not prepare them separately, so pho and noodle bowls are NOT celiac-safe here. Stick to rice plates.',
    whatToOrder: 'The rotisserie chicken with broken rice, or a rice plate — not the noodle dishes.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Open till midnight daily, later on weekends — but stick to the rice plates, the noodles are not celiac-safe',
    getThere: {
      transit: 'Metro Routes 60 and 3/4 on 12th Ave and Jefferson; a ~12-min walk from Capitol Hill Link',
      bike: 'The 12th Ave bike lane passes the door',
      parking: 'Free street parking on 12th Ave and the First Hill side streets after 8pm',
    },
  },
  {
    id: 'portage-bay-cafe-ballard',
    name: 'Portage Bay Cafe (Ballard)',
    neighborhood: 'Ballard',
    address: '2825 NW Market St, Seattle, WA 98107',
    lat: 47.6659,
    lng: -122.3969,
    cuisine: ['Brunch', 'American'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.portagebaycafe.com',
    phone: '(206) 783-1547',
    summary:
      'Popular brunch spot by the Ballard Locks with GF pancakes, French toast, and bread. The kitchen knows cross-contamination and servers are familiar with celiac; still, be explicit on a busy weekend.',
    whatToOrder: 'GF pancakes with the toppings bar, or an omelette with GF toast.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'RapidRide D and Route 40 stop on NW Market St right outside',
      bike: 'The Burke-Gilman Trail (missing-link detour) and NW Market St bike lanes are close',
      parking: 'Free street parking on NW Market and the side streets — tough on weekend brunch',
    },
  },
  {
    id: 'arayas-place-u-district',
    name: "Araya's Place",
    neighborhood: 'University District',
    address: '5240 University Way NE, Seattle, WA 98105',
    lat: 47.6668,
    lng: -122.3131,
    cuisine: ['Asian', 'Vegan & Veggie'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://arayasplace.com',
    phone: '(206) 524-4332',
    summary:
      "The Northwest's first vegan Thai restaurant (since 1987). Many curries and stir-fries are GF with tamari swapped in — ask for the GF guidance when ordering.",
    whatToOrder: 'Green curry, pad see ew with tamari, mango sticky rice.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'On the Ave (University Way NE) — many Metro routes; U District Link station is a ~8-min walk',
      bike: 'The Ave has bike lanes; the Burke-Gilman Trail is a few blocks south',
      parking: 'Paid street parking on the Ave and side streets; a couple of pay lots nearby',
    },
  },
  {
    id: 'frelard-tamales',
    name: 'Frelard Tamales',
    neighborhood: 'Fremont',
    address: '106 N 36th St, Seattle, WA 98103',
    lat: 47.6512,
    lng: -122.3493,
    cuisine: ['Mexican'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: 'https://www.frelardtamales.com',
    phone: '(206) 523-6654',
    summary:
      'Family and queer-owned tamale restaurant. Tamales are corn masa and gluten-free, and they say they do not use gluten in anything else they make (not a certified GF kitchen). Trusted by local celiacs.',
    whatToOrder: 'Pork in red sauce, the veggie tamale, rice and beans. Skip the croutoned salad and tres leches.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'On the Fremont/Ballard edge — Metro Routes 40 and 62 stop nearby on Leary Way / 36th',
      bike: 'A block off the Burke-Gilman Trail; easy ride from Fremont or Ballard',
      parking: 'Free street parking on the industrial side streets around N 36th St',
    },
  },
  {
    id: 'fonda-la-catrina',
    name: 'Fonda La Catrina',
    neighborhood: 'Georgetown',
    address: '5905 Airport Way S, Seattle, WA 98108',
    lat: 47.5477,
    lng: -122.3213,
    cuisine: ['Mexican'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.fondalacatrina.com',
    phone: '(206) 767-2787',
    summary:
      'Mexico City–style spot in Georgetown making tortillas, chips, and salsas in house. Corn-tortilla tacos and many plates are naturally GF; shared kitchen, so ask about mole and marinades.',
    whatToOrder: 'Tacos on corn tortillas, carne asada, elote.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 124 on Airport Way S stops in the Georgetown core',
      bike: 'Airport Way S bike lane connects Georgetown to SODO and the stadiums',
      parking: 'Free street parking throughout Georgetown, usually easy',
    },
  },
  {
    id: 'mioposto-mount-baker',
    name: 'Mioposto',
    neighborhood: 'Mount Baker',
    address: '3601 S McClellan St, Seattle, WA 98144',
    lat: 47.5806,
    lng: -122.292,
    cuisine: ['Italian', 'Pizza'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.miopostopizza.com',
    phone: '(206) 760-3400',
    summary:
      'Family-owned pizzeria by Mount Baker Park with a GF crust. They are upfront that there is flour in the air and they can\'t guarantee a GF environment — best for gluten-sensitive, riskier for celiac.',
    whatToOrder: 'GF Margherita or the seasonal pie; the chopped salad.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'A short walk from Mount Baker Link station and the Mount Baker transit hub',
      bike: 'The Mountains to Sound / Rainier Ave bike routes are close; Mount Baker Park is next door',
      parking: 'Free street parking around Mount Baker Park and the side streets',
    },
  },
  {
    id: 'nue-capitol-hill',
    name: 'Nue',
    neighborhood: 'Capitol Hill',
    address: '1519 14th Ave, Seattle, WA 98122',
    lat: 47.6145,
    lng: -122.3143,
    cuisine: ['Asian', 'American'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.nueseattle.com',
    phone: '(206) 257-0242',
    summary:
      'Global street-food small plates from a menu that spans continents, many dishes naturally gluten-free. Shared kitchen and fryer — ask the server which plates are safe and how they handle celiac.',
    whatToOrder: 'Rice-noodle dishes and grilled skewers; confirm sauces are GF.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'A ~10-min walk from Capitol Hill Link; Route 11 on E Pine St is close',
      bike: 'Off the Pike/Pine bike lanes; the 12th Ave lane is a block east',
      parking: 'Street only, paid until 10pm in this stretch of Pike/Pine',
    },
  },
  {
    id: 'tacos-chukis-broadway',
    name: 'Tacos Chukis (Broadway)',
    neighborhood: 'Capitol Hill',
    address: '219 Broadway E, Seattle, WA 98102',
    lat: 47.6194,
    lng: -122.3207,
    cuisine: ['Mexican'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 1,
    website: 'https://www.tacoschukis.com',
    phone: '(206) 328-4447',
    summary:
      'Beloved cheap taco counter hidden in the Broadway alley. Corn tortillas and grilled meats are naturally GF; flour tortillas are on the same line, so be clear.',
    whatToOrder: 'Adobada taco with grilled pineapple and cheese, on corn.',
    lastVerified: '2026-08',
    lateNight: true,
    lateNote: 'Open till 10pm every night — cheap late eats on Broadway',
    getThere: {
      transit: 'Steps from the Capitol Hill Link station and the Broadway streetcar',
      bike: 'On the Broadway protected bikeway',
      parking: 'Paid street parking on Broadway until 10pm; a pay garage is under the station',
    },
  },
  {
    id: 'sweet-alchemy-u-district',
    name: 'Sweet Alchemy Ice Creamery',
    neighborhood: 'University District',
    address: '4301 University Way NE, Seattle, WA 98105',
    lat: 47.6577,
    lng: -122.3135,
    cuisine: ['Bakery & Sweets'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 1,
    website: 'https://sweetalchemyicecreamery.com',
    phone: '(206) 812-0102',
    summary:
      'Small-batch organic ice cream with GF cones available and many GF flavors. Cookies and cones are made in-house on shared equipment — ask for a clean scoop.',
    whatToOrder: 'Honey lavender or Thai tea in a GF cone.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'On the Ave (University Way NE); U District Link station is a ~5-min walk',
      bike: 'The Ave bike lanes; the Burke-Gilman Trail is nearby',
      parking: 'Paid street parking on the Ave and side streets',
    },
  },
  {
    id: 'harvest-beat',
    name: 'Harvest Beat',
    neighborhood: 'Wallingford',
    address: '1711 N 45th St, Seattle, WA 98103',
    lat: 47.6612,
    lng: -122.3367,
    cuisine: ['Vegan & Veggie', 'American'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 4,
    website: 'https://www.harvestbeat.com',
    phone: '(206) 547-1348',
    summary:
      'Five-course plant-based tasting menu, one nightly seating at a chef\'s counter. They take dietary needs in advance and can run the whole menu gluten-free with notice.',
    whatToOrder: 'The set menu — tell them celiac when you book.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 44 runs on N 45th St through Wallingford past the door',
      bike: 'N 45th St and the Wallingford greenways connect here; a few blocks from the Burke-Gilman',
      parking: 'Free street parking on the Wallingford side streets',
    },
  },
  {
    id: 'cactus-madison-park',
    name: 'Cactus (Madison Park)',
    neighborhood: 'Madison Park',
    address: '4220 E Madison St, Seattle, WA 98112',
    lat: 47.6357,
    lng: -122.2795,
    cuisine: ['Mexican', 'American'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 3,
    website: 'https://www.cactusrestaurants.com',
    phone: '(206) 324-4140',
    summary:
      'Southwestern / Mexican with a dedicated gluten-intolerant menu; say celiac and they mark the order and take extra care. Not a dedicated facility — the corn chips are fried in oil shared with gluten items.',
    whatToOrder: 'GF enchiladas or plancha-seared dishes. Skip the chips if you are highly sensitive.',
    lastVerified: '2026-08',
    getThere: {
      transit: 'Metro Route 11 runs on E Madison St into Madison Park',
      bike: 'E Madison St bike lanes lead down to Madison Park and the beach',
      parking: 'Free street parking in the Madison Park village, tight on warm evenings',
    },
  },

  // ---------- Eastside ----------
  {
    id: 'cantina-monarca-bellevue',
    featured: true,
    name: 'Cantina Monarca',
    neighborhood: 'Bellevue',
    address: '504 Bellevue Way NE, Bellevue, WA 98004',
    lat: 47.6166,
    lng: -122.2029,
    cuisine: ['Mexican', 'Bars & Breweries'],
    safetyLevel: 'dedicated',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 3,
    website: 'https://www.cantinamonarca.com',
    phone: '(425) 531-7206',
    summary:
      "Downtown Bellevue's dedicated gluten-free kitchen — elevated Mexican and a tequila/mezcal lounge where everything but the bottled beer is GF. The chef nixtamalizes his own corn, the chips have their own fryer, and any gluten is handled only in the early morning. A co-owner's family is celiac. Note: 21+ after 6pm.",
    whatToOrder:
      'Shareable plates off the seasonal menu, birria, chips with the mole, and a mezcal flight.',
    lastVerified: '2026-09',
    lateNight: true,
    lateNote: 'Open till midnight Fri & Sat, 10:30pm weeknights — a rare fully GF late option on the Eastside',
    getThere: {
      transit: '2 Line light rail (Bellevue Downtown station) and the Bellevue Transit Center are both a ~5-min walk',
      bike: 'The 108th Ave NE bike lane and the Bellevue Downtown Park loop are a block away',
      parking: 'Free 3-hour parking in the Bellevue Collection garages across the street',
    },
  },
  {
    id: 'i-cant-believe-its-bread-bellevue',
    featured: true,
    name: "I Can't Believe It's Bread",
    neighborhood: 'Bellevue',
    address: '13600 NE 20th St, Ste B, Bellevue, WA 98005',
    lat: 47.6285,
    lng: -122.1386,
    cuisine: ['Bakery & Sweets'],
    safetyLevel: 'dedicated',
    dedicatedFryer: false,
    celiacVerified: true,
    priceLevel: 2,
    website: '',
    phone: '',
    order: false,
    summary:
      'A dedicated 100% gluten-free bakery and boutique market in Crossroads (formerly Wildflour) — house-baked sandwich breads, biscuits, cookies, and pastries, plus imported Italian GF pastas, sauces, and ramen and curry bowls to take home. Order-ahead pickup Mon–Sat.',
    whatToOrder: 'A sandwich loaf, the lemon poppyseed sweet bread, and a box of cookies for the freezer.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'Metro routes on 156th Ave NE and NE 20th St stop nearby; a few minutes from the Bel-Red 2 Line stations',
      bike: 'The NE 20th St and 156th Ave NE bike lanes run past the plaza',
      parking: 'Free lot in Apple Tree Plaza right out front',
    },
  },
  {
    id: 'joes-burgers-kirkland',
    name: "Joe's Burgers",
    neighborhood: 'Kirkland',
    address: '11910 NE Village Plaza, Kirkland, WA 98034',
    lat: 47.7062,
    lng: -122.1792,
    cuisine: ['American'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: true,
    celiacVerified: true,
    priceLevel: 2,
    website: '',
    phone: '(425) 576-0003',
    summary:
      'A walk-up burger counter at The Village at Totem Lake with a dedicated gluten-free fryer, dedicated GF fries, and GF buns that local celiacs rate as the best around. Covered patio with heaters and fire pits. Shared grill, so flag celiac when you order.',
    whatToOrder: 'A burger on the GF bun with a side of the dedicated-fryer fries and a shake.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'The Totem Lake / Kingsgate transit hub and RapidRide are right next to The Village',
      bike: 'The Cross Kirkland Corridor trail runs along the west edge of Totem Lake',
      parking: 'Free garage and surface parking throughout The Village at Totem Lake',
    },
  },
  {
    id: 'cafe-organique-kirkland',
    name: 'Café Organique',
    neighborhood: 'Kirkland',
    address: '11656 98th Ave NE, Kirkland, WA 98034',
    lat: 47.7046,
    lng: -122.1964,
    cuisine: ['Bowls & Salads', 'Vegan & Veggie', 'Brunch'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.cafeorganique.us',
    phone: '(425) 298-4575',
    summary:
      'An organic breakfast-and-lunch cafe that Eastside celiac families lean on — most of the menu is gluten-free, orders are prepped in a separate area, and the staff are fluent in cross-contamination. Weekdays 8am–3pm. Not a dedicated facility, so say celiac.',
    whatToOrder: 'A grain or salad bowl, the organic breakfast plate, and a fresh-pressed juice.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'Metro routes on 100th Ave NE and the Kingsgate P&R are a short walk',
      bike: 'The Cross Kirkland Corridor is close; quiet residential streets connect in',
      parking: 'Free lot and street parking around the building',
    },
  },
  // ---------- Mercer Island ----------
  {
    id: 'sano-cafe-mercer-island',
    name: 'Sano Cafe',
    neighborhood: 'Mercer Island',
    address: '7605 SE 27th St #111, Mercer Island, WA 98040',
    lat: 47.5704,
    lng: -122.2319,
    cuisine: ['Bowls & Salads', 'Brunch', 'Vegan & Veggie'],
    safetyLevel: 'celiac-friendly',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.thesanocafe.com',
    phone: '(206) 420-1663',
    summary:
      'A bright organic cafe in the Mercer Island town center where almost everything is gluten-free — overnight oats, smoothies, salads, lunch bowls, and avocado toast on GF bread. There is a separate prep area with its own knives and boards. Not a dedicated kitchen; tell them celiac.',
    whatToOrder: 'Avocado egg toast on GF bread, a lunch bowl, and a cold-pressed juice.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'The Mercer Island 2 Line light rail station is a ~10-min walk; several buses stop at the town center',
      bike: 'The I-90 Trail across Mercer Island connects Seattle and Bellevue a few blocks south',
      parking: 'Free 3-hour street and lot parking throughout the town center',
    },
  },
  // ---------- North ----------
  {
    id: 'beardslee-public-house-bothell',
    name: 'Beardslee Public House',
    neighborhood: 'Bothell',
    address: '19116 Beardslee Blvd #201, Bothell, WA 98011',
    lat: 47.7597,
    lng: -122.2086,
    cuisine: ['American', 'Bars & Breweries'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://beardsleeph.com',
    phone: '(425) 286-1001',
    summary:
      'A scratch-kitchen brewpub near Canyon Park with a dedicated gluten-free menu and prep notes, GF pizza, and Manini\'s GF burger buns. No dedicated fryer and gluten is in the kitchen — celiac reports are mostly good but not unanimous, so be explicit.',
    whatToOrder: 'A burger on the GF bun or the GF pizza; skip anything fried.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'Metro and Community Transit routes on Bothell-Everett Hwy and the Canyon Park P&R are close',
      bike: 'The Sammamish River Trail through downtown Bothell is about a mile south',
      parking: 'Free parking lot on site',
    },
  },
  // ---------- South ----------
  {
    id: 'theary-cambodian-foods-federal-way',
    name: 'Theary Cambodian Foods',
    neighborhood: 'Federal Way',
    address: '2012 S 320th St, Suite C, Federal Way, WA 98003',
    lat: 47.3179,
    lng: -122.3116,
    cuisine: ['Asian'],
    safetyLevel: 'gf-menu',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 1,
    website: 'https://theary-cambodian-foods.square.site',
    phone: '(206) 201-9995',
    summary:
      'A woman-owned Cambodian takeout counter where most of the rice- and rice-noodle-based menu is naturally gluten-free and the owner is used to celiac questions. Takeout only, shared kitchen — ask which curries and sauces are safe.',
    whatToOrder: 'Lemongrass chicken or beef lok lak over rice; confirm the marinade and dipping sauces.',
    lastVerified: '2026-09',
    getThere: {
      transit: 'RapidRide A Line on Pacific Hwy S and the Federal Way Transit Center are a ~10-min walk',
      bike: 'S 320th St has bike lanes connecting to the BPA Trail',
      parking: 'Free strip-mall lot out front',
    },
  },

  // ---------- Honorable mentions ----------
  // Not gluten-free kitchens. Healthy, affordable, lots of naturally GF options.
  // Cross-contamination is likely — a low-risk quick bite, not a celiac-safe meal.
  {
    id: 'hm-chipotle-downtown',
    honorableMention: true,
    name: 'Chipotle Mexican Grill',
    neighborhood: 'Downtown',
    address: '1501 4th Ave, Seattle, WA 98101',
    lat: 47.6106,
    lng: -122.3365,
    cuisine: ['Mexican', 'Bowls & Salads'],
    safetyLevel: 'honorable',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 1,
    website: 'https://www.chipotle.com',
    phone: '',
    summary:
      'A burrito bowl with rice, beans, meat, salsa, and guac is naturally gluten-free and cheap. Staff will change gloves if you ask. Shared line with flour tortillas — a reliable default, not for the highly sensitive.',
    whatToOrder: 'Bowl, no tortilla. Ask for fresh gloves and a scoop from the back.',
    lastVerified: '2026-08',
  },
  {
    id: 'hm-evergreens-uvillage',
    honorableMention: true,
    name: 'Evergreens',
    neighborhood: 'University Village',
    address: '2623 NE University Village St, Seattle, WA 98105',
    lat: 47.6626,
    lng: -122.2996,
    cuisine: ['Bowls & Salads'],
    safetyLevel: 'honorable',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://evergreens.com',
    phone: '',
    summary:
      'Seattle-grown salad chain. Build a bowl and skip the croutons/wontons and you are largely gluten-free. Shared prep area and some dressings contain gluten — check the allergen guide.',
    whatToOrder: 'Custom salad or grain bowl, gluten-containing toppings left off.',
    lastVerified: '2026-08',
  },
  {
    id: 'hm-sweetgreen-slu',
    honorableMention: true,
    name: 'sweetgreen',
    neighborhood: 'South Lake Union',
    address: '801 Lenora St, Seattle, WA 98121',
    lat: 47.6165,
    lng: -122.3395,
    cuisine: ['Bowls & Salads'],
    safetyLevel: 'honorable',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://www.sweetgreen.com',
    phone: '',
    summary:
      'Salads and warm grain bowls with a detailed online allergen filter. Most builds are gluten-free if you avoid the focaccia and a couple of dressings. Shared assembly line.',
    whatToOrder: 'Use the app allergen filter, then a grain or salad bowl.',
    lastVerified: '2026-08',
  },
  {
    id: 'hm-fob-poke-bar',
    honorableMention: true,
    name: 'FOB Poke Bar',
    neighborhood: 'Belltown',
    address: '220 Blanchard St, Seattle, WA 98121',
    lat: 47.6146,
    lng: -122.3455,
    cuisine: ['Asian', 'Bowls & Salads', 'Seafood'],
    safetyLevel: 'honorable',
    dedicatedFryer: false,
    celiacVerified: false,
    priceLevel: 2,
    website: 'https://fobpokebar.com',
    phone: '(206) 728-9888',
    summary:
      'Poke bowls over rice are naturally gluten-free if you skip the shoyu/ponzu and crispy onions and ask for coconut aminos or salt. Soy sauce is all over the line, so be clear.',
    whatToOrder: 'Rice bowl, tuna/salmon, no soy-based sauce, no crispy toppings.',
    lastVerified: '2026-08',
  },
];
