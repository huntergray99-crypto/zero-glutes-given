...
2. Go to https://www.pwabuilder.com → enter `https://zeroglutesgiven.culebramaps.com` → **Package for stores → Android**.
...
3. Point the wrapper at the live site (in `capacitor.config.json`):
   ```json
   { "server": { "url": "https://zeroglutesgiven.culebramaps.com" } }
   ```
